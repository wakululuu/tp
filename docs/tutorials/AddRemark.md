...
/**
 * Changes the remark of an existing worker in the address book.
 */
...
``` java
    public static final String MESSAGE_USAGE = COMMAND_WORD + ": Edits the remark of the worker identified "
            + "by the index number used in the last worker listing. "
...
public class RemarkCommand extends Command {
    ...
    /**
     * @param index of the worker in the filtered worker list to edit the remark
     * @param remark of the worker to be updated to
     */
...
Create a new `Remark` in `seedu.address.model.worker`. Since a `Remark` is a field that is similar to `Address`, we can reuse a significant bit of code.
...
Without getting too deep into `fxml`, let’s go on a 5 minute adventure to get some placeholder text to show up for each worker.

Simply add the following to [`seedu.address.ui.WorkerCard`](https://github.com/se-edu/addressbook-level3/commit/850b78879582f38accb05dd20c245963c65ea599#diff-0c6b6abcfac8c205e075294f25e851fe).
...
Then insert the following into [`main/resources/view/WorkerListCard.fxml`](https://github.com/se-edu/addressbook-level3/commit/850b78879582f38accb05dd20c245963c65ea599#diff-12580431f55d7880578aa4c16f249e71).
...
## Modify `Worker` to support a `Remark` field

Since `WorkerCard` displays data from a `Worker`, we need to update `Worker` to get our `Remark` displayed!

### Modify `Worker`

We change the constructor of `Worker` to take a `Remark`. We will also need to define new fields and accessors accordingly to store our new addition.

### Update other usages of `Worker`

Unfortunately, a change to `Worker` will cause other commands to break, you will have to modify these commands to use the updated `Worker`!

<div markdown="span" class="alert alert-primary">

:bulb: Use the `Find Usages` feature in IntelliJ IDEA on the `Worker` class to find these commands.
...
AddressBook stores data by serializing `JsonAdaptedWorker` into `json` with the help of an external library — Jackson. Let’s update `JsonAdaptedWorker` to work with our new `Worker`!
...
Now that we have finalized the `Worker` class and its dependencies, we can now bind the `Remark` field to the UI.
...
public WorkerCard(Worker worker, int displayedIndex) {
    //...
    remark.setText(worker.getRemark().value);
}
```
...
In this last step, we modify `RemarkCommand#execute()` to change the `Remark` of a `Worker`. Since all fields in a `Worker` are immutable, we create a new instance of a `Worker` with the values that we want and
save it with `Model#setWorker()`.

**`RemarkCommand.java`:**

``` java
//...
    public static final String MESSAGE_ADD_REMARK_SUCCESS = "Added remark to Worker: %1$s";
    public static final String MESSAGE_DELETE_REMARK_SUCCESS = "Removed remark from Worker: %1$s";
//...
    @Override
    public CommandResult execute(Model model) throws CommandException {
        ...
        Worker editedWorker = new Worker(workerToEdit.getName(), workerToEdit.getPhone(), workerToEdit.getEmail(),
                workerToEdit.getAddress(), remark, workerToEdit.getTags());

        model.setWorker(workerToEdit, editedWorker);
        model.updateFilteredWorkerList(PREDICATE_SHOW_ALL_WORKERS);

        return new CommandResult(generateSuccessMessage(editedWorker));
    }
     ...
     * {@code workerToEdit}.
     */
    private String generateSuccessMessage(Worker workerToEdit) {
        String message = !remark.value.isEmpty() ? MESSAGE_ADD_REMARK_SUCCESS : MESSAGE_DELETE_REMARK_SUCCESS;
        return String.format(message, workerToEdit);
    }
```
