...
``` java
public PersonCard(Person worker, int displayedIndex) {
    //...
...
//...
    ...
    public CommandResult execute(Model model) throws CommandException {
        ...
        Person workerToEdit = lastShownList.get(index.getZeroBased());
        Person editedWorker = new Person(workerToEdit.getName(), workerToEdit.getPhone(), workerToEdit.getEmail(),
                workerToEdit.getAddress(), remark, workerToEdit.getTags());

        model.setPerson(workerToEdit, editedWorker);
     ...
     * {@code workerToEdit}.
     */
    private String generateSuccessMessage(Person workerToEdit) {
